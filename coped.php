...
<body>
        ...
        fetch('ver_mensagem.php?id=' + id)
            ...
            .then(data => {
                if (data.success) {
                    ...
                    // Atualizar o status da mensagem na interface
    ...
    function aprovarReservaSala(id) {
        if(confirm('Confirmar aprovação da reserva de sala?')) {
    ...
    function recusarReservaSala(id) {
        if(confirm('Confirmar recusa da reserva de sala?')) {
    ...
    $(document).ready(function() {
        // Excluir reserva
        $('.excluir-reserva').click(function() {
            const id = $(this).data('id');
            if (confirm('Tem certeza que deseja excluir esta reserva?')) {
                $.post('excluir_reserva.php', { id: id }, function(response) {
                    if (response.success) {
                        alert('Reserva excluída com sucesso!');
                        location.reload();
                    } else {
                        alert('Erro ao excluir reserva: ' + response.message);
                    }
                }, 'json');
            }
        });
        ...
        $('.excluir-reserva-sala').click(function() {
            const id = $(this).data('id');
            if (confirm('Tem certeza que deseja excluir esta reserva de sala?')) {
                $.post('excluir_reserva_sala.php', { id: id }, function(response) {
                    if (response.success) {
                        alert('Reserva de sala excluída com sucesso!');
                        location.reload();
                    } else {
                        alert('Erro ao excluir reserva de sala: ' + response.message);
                    }
                }, 'json');
            }
        });
        ...
        $('.excluir-mensagem').click(function() {
            ...
            if (confirm('Tem certeza que deseja excluir esta mensagem?')) {
                $.post('excluir_mensagem.php', { id: id }, function(response) {
                    if (response.success) {
                        $(this).closest('tr').fadeOut(400, function() {
                            $(this).remove();
                        });
                    } else {
                        alert('Erro ao excluir mensagem: ' + response.message);
                    }
                }, 'json')
                .fail(function() {
